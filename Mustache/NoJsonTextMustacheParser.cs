    ...
    public static class Parser {
        ...
        static (Collection, IEnumerator<Token>) HandleOpenCollection(Token open, IEnumerator<Token> rest, Collection tree) {
            ...
            if(open.Value.Length == 0)
                return (c, rest); // top value collection
            else
        ...
        static string FindProperty(string name, ImmutableStack<object> path) {
            while(!path.IsEmpty) {
                path = path.Pop(out var el);
                var d = el as Dictionary<string,object>; 
                if(d != null) { 
                    var found = d.TryGetValue(name, out var val);
                    if(found) return val!.ToString()!;
                }
            }
            return "";
        }
        static StringBuilder RenderNode(Node n, StringBuilder sb, ImmutableStack<object> path) => n switch
        {
        ...
        static StringBuilder RenderCollection(Collection c, StringBuilder sb, ImmutableStack<object> path) {
            
            var coll = (path.Peek() as Dictionary<string, object>);
            if(coll is null) throw new Exception("Shouldn't get here as passing a collection ...");
            var found = coll.TryGetValue(c.Name, out var el);
            if(!found) return sb;

            path = path.Push(el!);

            if(el is bool && (bool)el == false) return sb;

            if(el is List<object> && (el as List<object>)!.Count == 0) return sb;
            if(el is bool && (bool)el == true) { // Object for initial collection
                sb = RenderObject(c, sb, path);
            }
            // . Otherwise for each object render node with json object updated to current
            else if(el is List<object>) {
                foreach (var subEl in (el as List<object>)!) {
        ...
        static StringBuilder RenderObject(Collection c, StringBuilder sb, ImmutableStack<object> path) {
                foreach (var n in c.Nodes) {
        ...
        public static string Render(Collection c, string jsonText) {
            ...
            var vars = jsonText.FromJson<object>();
            var sb = new StringBuilder();
            sb = RenderObject(c, sb, ImmutableStack.Create<object>(vars));
            var s = sb.ToString();
            return s;
        }
...
// From: https://github.com/zanders3/json
